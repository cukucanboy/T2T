...
class Home extends MX_Controller {
		    ...
		    $this->lang->load("front", $this->data['lang_set']);
				...
				if ($pageslug == null || $this->validlang && empty($secondslug)) {
						...
						//$this->data['featuredSection']['modules'] = array();
												...
												if (pt_main_module_available('advertising')) {
														$activeModules[] = "advertising";
														$this->load->library('Advertising/Advertising_lib');
														$this->data['advertisinglib'] = $this->Advertising_lib;
												...
												//	  $this->data['destinationList'] = $this->Advertising_lib->getDestinationList();
														$this->data['featuredAdvertising'] = $this->Advertising_lib->getFeaturedAdvertising();
														$this->data['popularAdvertising'] = $this->Advertising_lib->getTopRatedAdvertising();
														...
														$this->data['featuredSection']['modules']["advertising"] = (object)array("featured" => $this->Advertising_lib->getFeaturedAdvertising(),'moduleTitle' => trans('Advertising'), 'bgImg' => 'featured-advertising.jpg', 'booknowClass' => 'warning','featuredText' => trans('0451'), 'featuredPrice' => 200,'currCode' => 'THB');
														$this->data['advertisingLocations'] = $this->Advertising_lib->advertisingByLocations();

														$this->load->helper("Advertising/advertising_front");
        ...
        if (pt_main_module_available('blog')) {
						...
						if (pt_main_module_available('destinations')) {
										$this->load->library('Destinations/Destinations_lib');
										$this->data['destinationlib'] = $this->Destinations_lib;
										$this->load->helper("Destinations/destinations_front");
										$destinations = $this->Destinations_lib->getDestinationHomepage();
										$this->data['destinationList'] = $destinations->destinations;
										$settings = $this->Destinations_lib->settings();
										$this->data['showOnHomePage'] = $settings[0]->front_homepage_hero;
								}
