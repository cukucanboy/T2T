...
// get car images
    ...
    function car_details($carid = null)
    {
        ...
        $ogimg = $this->ogimg($details[0]->car_id);
    ...
    function ogimg($carid)
    {
        if (empty($carid)) {
            $carid = $this->carid;
        }
        $this->db->where('cimg_car_id', $carid);
        $this->db->where('cimg_order', '1');
        $res = $this->db->get('pt_car_images')->result();
        if (empty($res)) {
            $result = 'uploads/global/favicon.png';
        } else {
            $result =  'uploads/images/cars/slider/thumbs/'.$res[0]->cimg_image;
        }
        return $result;
    }

    function get_metaImg($defimg, $carid = null)
    {
        if (empty($carid)) {
            $carid = $this->carid;
        }
