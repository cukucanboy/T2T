...
// add car data
        ...
        $paymentopt =  @implode(",",$this->input->post('carpayments'));
				...
				$data = array(
				'car_title' => $this->input->post('carname'),
				'car_slug' => $carslug,
...
// update car data
		...
		function convert_json($nearbyrelatedcar)
	  {
	    $return_js = [];
	    $data = explode(',',$nearbyrelatedcar);
	    foreach ($data as $key) {
	      //print_r($key);
	      $getmd = substr($key,-2);
...
//$days = pt_count_days($checkin,$checkout);
				...
				if(!empty($pickupLocation) && $pickupLocation != 'Pick up Location' && !empty($dropoffLocation) &&  $dropoffLocation!= 'Drop off Location'){
