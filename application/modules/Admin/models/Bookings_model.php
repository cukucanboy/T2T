...
class Bookings_model extends CI_Model
{
    ...
    {
        $this->db->select('pt_bookings.booking_member_id,pt_bookings.booking_cancellation_request,pt_bookings.booking_id,pt_bookings.booking_type,pt_bookings.booking_expiry,pt_bookings.booking_ref_no,
        ...
        $this->db->join('pt_accounts', 'pt_bookings.booking_member_id = pt_accounts.accounts_id', 'left');
    ...
    function do_login_booking($username, $password)
    {
        ...
        if ($login) {
            ...
            return $this->do_booking($userid, NULL, 'N');
        } else {
    ...
    function doGuestBooking($bookquick = null)
    {
        ...
        $user = $this->Accounts_model->guest_booking('guest', 'YES');
        if($user){
            $guestid = $this->session->userdata('pt_logged_guest');
            if (empty($bookquick)) {
                return $this->do_booking(NULL, $user,'Y');
            } else {
                return $this->doQuickBooking(NULL, $user,'Y');
            }
        }else {
            $bookingResult = array("error" => "yes", 'msg' => 'Something Error.');
            return $bookingResult;
        }

    }

    function do_booking($userid, $guestid, $isGuest)
    {
        ...
        if ($bookingtype == "hotels") {
            ...
            $ctmcheckinvar = $this->input->post('ctmcheckin');
            $ctmcheckoutvar = $this->input->post('ctmcheckout');

            $date1 = str_replace('/', '-', $ctmcheckinvar);
            $date2 = str_replace('/', '-', $ctmcheckoutvar);
            ...
            $ctmcheckin = date('Y-m-d', strtotime($date1));
            $ctmcheckout = date('Y-m-d', strtotime($date2));
            ...
            $ccheckinvar = $this->input->post('ctmcheckin');
            $ccheckoutvar = $this->input->post('ctmcheckout');

            $date1 = str_replace('/', '-', $ccheckinvar);
            $date2 = str_replace('/', '-', $ccheckoutvar);
        ...
        if (!$error) {
            $data = array(
                ...
                'booking_customer_hotel_name' => $hotelname,
                'booking_customer_checkin' => $ctmcheckin,
                'booking_customer_checkout' => $ctmcheckout
            );
            ...
            $url = base_url() . 'invoice?id=' . $bookid . '&sessid=' . $refno .'&member=' .$isGuest;
            $bookingResult = array("error" => "no", 'msg' => '', 'url' => $url);
            $invoicedetails = invoiceDetails($bookid, $refno, $isGuest);

            $this->Emails_model->sendEmail_customer($invoicedetails, $this->data['app_settings'][0]->site_title, $isGuest);
            $this->Emails_model->sendEmail_admin($invoicedetails, $this->data['app_settings'][0]->site_title, $isGuest);
            $this->Emails_model->sendEmail_owner($invoicedetails, $this->data['app_settings'][0]->site_title, $isGuest);
            $this->Emails_model->sendEmail_supplier($invoicedetails, $this->data['app_settings'][0]->site_title, $isGuest);
...
//Do quick booking by admin
    function doQuickBooking($userid, $guestid, $isGuest)
    {
        $this->load->library('currconverter');
        $itemid = $this->input->post('itemid');
        ...
        if ($bookingtype == "hotels") {
            $rdata = array(
                'booked_booking_id' => $bookid,
                'booked_room_id' => $subitemid,
                'booked_room_count' => $roomscount,
                'booked_checkin' => $checkin,
                'booked_checkout' => $checkout,
                'booked_booking_status' => 'unpaid',
                'isGuest' => 'N');
            ...
            $cdata = array(
                'booked_booking_id' => $bookid,
                'booked_car_id' => $this->input->post('itemid'),
                'booked_checkin' => convert_to_unix($checkin),
                'booked_checkout' => convert_to_unix($checkout),
                'booked_booking_status' => 'unpaid');
            $this->db->insert('pt_booked_cars', $cdata);
        }
    ...
    {
        $this->db->select('booking_member_id');
        $this->db->where('booking_id', $id);
        $res = $this->db->get('pt_bookings')->result();
        $user = $res[0]->booking_member_id;
