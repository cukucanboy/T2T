...
class Payments_model extends CI_Model
{
    ...
    function getGatewayMsg($gateway, $invoicvars)
    {
        ...
        $params['paidAmount'] = $invoicvars->paidAmount;
        $params['currency'] = $invoicvars->currCode;
        $params['description'] = $this->getOrderTitle($invoicvars->module ,$invoicvars->itemid);//$this->settings[0]->site_title;
    ...
    function getOrderTitle($module ,$itemid)
    {
        $modules = substr($module, 0, -1);

        $this->db->select($modules.'_title');
        $this->db->where($modules.'_id', $itemid);
        $data = $this->db->get('pt_'.$module)->result();

        $title = $data[0]->hotel_title;
        return $title;
    }
