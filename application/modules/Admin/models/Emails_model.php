...
// Call the Model constructor
        ...
        $this->sysemail = $this->get_sys_email();
...
//send email to customer
    function sendEmail_customer($details, $sitetitle, $isGuest)
    {
        ...
        $invoicelink = "<a href=" . base_url() . "invoice?id=" . $invoiceid . "&sessid=" . $refno . "&member=" . $isGuest . " > More Booking Information </a>";
...
//send email to Admin
    function sendEmail_admin($details, $sitetitle, $isGuest)
    {
        $currencycode = $details->currCode;
        $currencysign = $details->currSymbol;
...
//send email to Owner
    function sendEmail_owner($details, $sitetitle, $isGuest)
    {
        ...
        $message .= "<br> To view Invoice visit at: <a href=" . base_url() . "invoice?id=" . $invoiceid . "&sessid=" . $refno . "&member=" . $isGuest . " >More Booking Information</a>";
        $message .= $this->mailFooter;
...
//send email to Supplier
    function sendEmail_supplier($details, $sitetitle, $isGuest)
    {
        $currencycode = $details->currCode;
        $currencysign = $details->currSymbol;
...
//send email to Owner for booking paid
    function paid_sendEmail_owner($details)
    {
        ...
        $message .= "Country : " . $country . "<br>";
        $message .= "Phone : " . $phone . "<br>";
        $message .= "<br> To view Invoice <a href=" . base_url() . "invoice?id=" . $invoiceid . "&sessid=" . $refno . "&member=" . $isGuest . " >More Booking Information</a>";
        $message .= $this->mailFooter;
        $this->email->set_newline("\r\n");
        $this->email->from($this->sendfrom);
        $this->email->to($sendto);
...
// get admin email
    ...
    function get_admin_email()
    {
        ...
        return $q[0]->admin_email;
    }

    // get sys email
    function get_sys_email()
    {
        $this->db->select('contact_email');
        //$this->db->where('accounts_type', 'webadmin');
        $q = $this->db->get('pt_contact_page')->result();
        return $q[0]->contact_email;
    }
...
// send New supplier signup email to Admin
    ...
    public function new_supplier_email($edata)
    {
        ...
        $values = array($edata['fullname'], $edata['email'], $edata['address'], $edata['phone']);
        ...
        $this->email->set_newline("\r\n");
        $this->email->from($this->sendfrom);
        $this->email->to($this->adminemail); // adminemail
        $this->email->subject($details[0]->temp_subject);
        $this->email->message($message);
        $this->email->send();
...
// send New customer signup email to Admin
    ...
    public function new_customer_email($edata)
    {
        ...
        //$smsdetails = sms_template_detail("supplierregisteradmin");
        $values = array($edata['fullname'], $edata['email'], NULL, $edata['phone']);
