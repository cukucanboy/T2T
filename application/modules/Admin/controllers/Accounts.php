...
// Guest function
    ...
    public function guest($args = null, $id = null)
    {
        ...
        $userdata = $this->Accounts_model->get_guest_profile_details($id);
        if (!empty($userdata)) {
            $this->data['profile'] = $userdata;
            $this->data['isSubscribed'] = $this->Newsletter_model->is_subscribed($this->data['profile'][0]->accounts_email);
            $this->data['permitted'] = explode(",", $this->data['profile'][0]->accounts_permissions);
        ...
        if ($args == 'add') {
            ...
            if (!empty ($updateaccount)) {
                ...
                if ($this->form_validation->run('updateaccount') == FALSE) {
                    ...
                    if (isset ($_FILES['photo']) && !empty ($_FILES['photo']['name'])) {
                        ...
                        $this->Accounts_model->update_profile_guest($id, $filename_db);
            ...
            $xcrud = xcrud_get_instance();
            $xcrud->table('pt_guest_accounts');
            //$xcrud->where('accounts_type =', 'guest');
            $xcrud->order_by('guest_id', 'desc');
            $xcrud->button(base_url() . 'admin/accounts/guest/edit/{guest_id}', 'Edit', 'fa fa-edit', 'btn btn-warning', array('target' => '_self'));
            $delurl = base_url() . 'admin/ajaxcalls/delAcc';
            $xcrud->button("javascript: delfunc('{guest_id}','$delurl')", 'DELETE', 'fa fa-times', 'btn-danger', array('target' => '_self', 'id' => '{guest_id}'));
            $xcrud->limit(50);
            $xcrud->unset_add();
            $xcrud->unset_edit();
            $xcrud->unset_remove();
            $xcrud->unset_view();
            $xcrud->columns('ai_first_name,ai_last_name,guest_email,guest_status');
            $xcrud->search_columns('ai_first_name,ai_last_name,guest_email,guest_status');
            $xcrud->column_callback('guest_status', 'create_status_icon');
            //$xcrud->column_callback('guest_last_login', 'long_date_fmt');
            $xcrud->label('ai_first_name', 'First Name')->label('ai_last_name', 'Last Name')->label('guest_email', 'Email')->label('guest_status', 'Active');
...
// Suppliers function
    ...
    public function suppliers($args = null, $id = null)
    {
        ...
        if ($args == 'add') {
            ...
            $this->data['chkingadvertising'] = $this->ptmodules->is_mod_available_enabled("advertising");
            ...
            if ($this->data['chkingadvertising']) {
                $this->load->model('Advertising/Advertising_model');
                $this->data['useradvertising'] = array();
                $this->data['advertising'] = $this->Advertising_model->all_advertising_names();
            }
            ...
            if ($this->data['chkingadvertising']) {
                $this->load->model('Advertising/Advertising_model');
                $this->data['useradvertising'] = $this->Advertising_model->userOwnedAdvertising($id);
                $this->data['advertising'] = $this->Advertising_model->all_advertising_names();
            }
