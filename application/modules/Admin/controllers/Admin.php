    ...
    public function index()
    {
        ...
        if ($license || in_array($_SERVER['REMOTE_ADDR'], $whitelist) || $_SERVER['HTTP_HOST'] == "server") {
            if ($this->validadmin()) {
                ...
                $this->data['graphReportAdvertising'] = modules:: run('Admin/reports/graphReport', 'advertising');
...
//refresh last activity
    ...
    function advertising($args = null, $id = null, $lang = null)
    {
        $advertisingmod = modules:: load('advertising/advertisingback/');
        if (!method_exists($advertisingmod, 'index')) {
            redirect('admin');
        }
        if ($args == "") {
            $advertisingmod->index();
        } elseif ($args == "add") {
            $advertisingmod->add();
        } elseif ($args == "settings") {
            $advertisingmod->settings();
        } elseif ($args == "manage") {
            $advertisingmod->manage($id);
        } elseif ($args == "extras") {
            $advertisingmod->extras($id);
        } elseif ($args == "reviews") {
            $advertisingmod->reviews($id);
        } elseif ($args == "gallery") {
            $advertisingmod->gallery($id);
        } elseif ($args == "translate") {
            $advertisingmod->translate($id, $lang);
        }
    }
    ...
    {
        $restaurantmod = modules:: load('restaurant/restaurantback/');
        if (!method_exists($restaurantmod, 'index')) {
            redirect('admin');
        }
        if ($args == "") {
            $restaurantmod->index();
            ...
            $restaurantmod->settings();
        } elseif ($args == "manage") {
            $restaurantmod->manage($id);
        } elseif ($args == "extras") {
            $restaurantgmod->extras($id);
        } elseif ($args == "reviews") {
            $restaurantmod->reviews($id);
        } elseif ($args == "gallery") {
            $restaurantmod->gallery($id);
        } elseif ($args == "translate") {
            $restaurantmod->translate($id, $lang);
        }
    }
...
//Entertainment module controller
    function entertainment($args = null, $id = null, $lang = null)
    {
        $entertainmentmod = modules:: load('entertainment/entertainmentback/');
        if (!method_exists($entertainmentmod, 'index')) {
            redirect('admin');
        }
        if ($args == "") {
            $entertainmentmod->index();
        } elseif ($args == "add") {
            $entertainmentmod->add();
        } elseif ($args == "settings") {
            $entertainmentmod->settings();
        } elseif ($args == "manage") {
            $entertainmentmod->manage($id);
        } elseif ($args == "extras") {
            $entertainmentmod->extras($id);
        } elseif ($args == "reviews") {
            $entertainmentmod->reviews($id);
        } elseif ($args == "gallery") {
            $entertainmentmod->gallery($id);
        } elseif ($args == "translate") {
            $entertainmentmod->translate($id, $lang);
        }
    }
...
// Destinations module controller
    function Destinations($args = null, $id = null, $lang = null)
    {
        $destinationmod = modules:: load('destinations/destinationsback/');
        if (!method_exists($destinationmod, 'index')) {
            redirect('admin');
        }
        if ($args == "") {
            $destinationmod->index();
        } elseif ($args == "add") {
            $destinationmod->add();
        } elseif ($args == "category") {
            $destinationmod->category();
        } elseif ($args == "settings") {
            $destinationmod->settings();
        } elseif ($args == "manage") {
            $destinationmod->manage($id);
        } elseif ($args == "translate") {
            $destinationmod->translate($id, $lang);
        }
    }


// Wish module controller
    function Wish($args = null, $id = null, $lang = null)
    {
        $wishmod = modules:: load('wish/wishback/');
        if (!method_exists($wishmod, 'index')) {
            redirect('admin');
        }
        if ($args == "") {
            $wishmod->index();
            ...
            $wishmod->translate($id, $lang);
        }
    }

    // Wish module controller
    function Payment($args = null, $id = null, $lang = null)
    {
        $paymentmod = modules:: load('payment/paymentback/');
        if (!method_exists($paymentmod, 'index')) {
            redirect('admin');
        }
        if ($args == "") {
            $paymentmod->index();
            ...
            $paymentmod->manage($id);
        } elseif ($args == "translate") {
            $paymentmod->translate($id, $lang);
        }
    }
