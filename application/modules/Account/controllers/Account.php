...
//$perpage = 10;
        ...
        $this->data['points'] = $this->Accounts_model->get_my_points($this->loggedin);
    ...
    function signup()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($nums > 0) {
                ...
                $edata = array("email" => $this->input->post('email'), "fullname" => $fullname, "phone" => $this->input->post('phone'), "password" => $this->input->post('password'));
                if ($accStatus == "no") {
                    $this->Emails_model->new_customer_email($edata); // mail to admin
                    $this->Emails_model->customer_signup($edata); // mail to customer
                } else {
                    //echo "<div class='alert alert-success'>email not send to admin!</div>";
                    $this->Emails_model->new_customer_email($edata); // mail to admin
                    $this->Emails_model->signupEmail($edata); // mail to customer
