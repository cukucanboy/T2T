...
function paypalpaymentspro_capture($params, $auth = "") {
	...
	$paymentvars['PARTNER'] = "PayPal";


	//$paymentvars['USER'] = 'admin_api1.hostkonthai.com';
	//$paymentvars['PWD'] = 'HKU9KKENTNAJ5EZ8';
	//$paymentvars['SIGNATURE'] = 'A3LzFFXg3nbNRYIbcdZFEx5td9uzAs1IsGRaMFG5Pff1dZJDII5.Gm3f';
	
	$paymentvars['PWD'] = $params['apipassword'];
	$paymentvars['USER'] = $params['apiusername'];
	$paymentvars['SIGNATURE'] = $params['apisignature'];

	$paymentvars['IPADDRESS'] = $_SERVER['REMOTE_ADDR'];
	$paymentvars['PAYMENTACTION'] = "Sale";
	$paymentvars['CURRENCY'] = 'USD';
	$paymentvars['AMT'] = $params['amount'];
	$paymentvars['CREDITCARDTYPE'] =  'MasterCard';
	$paymentvars['ACCT'] = $params['cardnum'];
	$paymentvars['EXPDATE'] = substr($params['cardexp'], 0, 2) . "" . substr($params['cardexp'], 3, 4);
	...
	if ($params['cardstart']) {
		$paymentvars['STARTDATE'] = substr($params['cardstart'], 0, 2) . "" . substr($params['cardstart'], 3, 4);
	}
	...
	//$paymentvars['COUNTRYCODE'] = 'TH'; 

	$paymentvars['CURRENCYCODE'] = 'USD';
...
echo "<pre>";
     print_r($paymentvars);
  echo "</pre>";
//var_dump($paymentvars);

	$response = curlCall($url, $paymentvars);
//$response = curlCall($url, $request_params);
