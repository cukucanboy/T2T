...
// get hotel images
  function hotelImages($hotelid = 0) {
    if (empty($hotelid)) {
...
//	$this->isfeatured = $this->is_featured();
    ...
    $detailResults = (object) array('id' => $details[0]->hotel_id,
...
//	$this->bestprice = $details[0]->hotel_best_price;
  ...
  function get_policy($defpolicy, $hotelid = 0) {
    if (empty($hotelid)) {
      $hotelid = $this->hotelid;
    }
    if ($this->lang == $this->langdef) {
...
//make a result object all data of hotels array
  function getResultObject($hotels) {
    ...
    foreach ($hotels as $h) {
      ...
      $priceRange = $this->priceRange($price); // Update  From  $_GET['price']
      if (!empty($_GET['price'])) {
