...
function fmtDate2($value, $fieldname, $primary_key, $row, $xcrud)
{
...
//Mod #1 Activity
function activityGallery($value, $fieldname, $primary_key, $row, $xcrud)
{
    $photocounts = pt_ActivityPhotosCount($primary_key);
    $CI = get_instance();
    $role = $CI->session->userdata('pt_role');
    if ($role != "supplier") {
        $role = "admin";
    }
    return "<a href=" . base_url() . $role . "/activity/gallery/" . $value . ">Upload (" . $photocounts . ")</a>";
}

function orderInputActivity($value, $fieldname, $primary_key, $row, $xcrud)
{
    $url = base_url() . "activity/activityajaxcalls/update_activity_order";

    return '<input class="form-control input-sm" data-url=' . $url . ' type="number" id="order_' . $primary_key . '" value=' . $value . ' min="1"  onblur="updateOrder($(this).val(),' . $primary_key . ',' . $value . ')" />';
}

function feature_starsActivity($value, $fieldname, $primary_key, $row, $xcrud)
{
    $url = base_url() . "activity/activityajaxcalls/update_featured";
    if ($value == "yes") {
        return "<span class='star fa fa-star' onclick='fstar(\"$url\",\"no\",\"$primary_key\")' id=\"$primary_key\"  style='cursor: pointer;'></span>";
    } else {
        return "<span class='fa fa-star-o fstar' onclick='fstar(\"$url\",\"yes\",\"$primary_key\")' id=\"$primary_key\" style='cursor: pointer;' ></span>";
...
//Mod #2 Entertainment
function entertainmentGallery($value, $fieldname, $primary_key, $row, $xcrud)
{
    $photocounts = pt_EntertainmentPhotosCount($primary_key);
    $CI = get_instance();
    $role = $CI->session->userdata('pt_role');
    if ($role != "supplier") {
        $role = "admin";
    }
    return "<a href=" . base_url() . $role . "/entertainment/gallery/" . $value . ">Upload (" . $photocounts . ")</a>";
}

function orderInputEntertainment($value, $fieldname, $primary_key, $row, $xcrud)
{
    $url = base_url() . "entertainment/entertainmentajaxcalls/update_entertainment_order";

    return '<input class="form-control input-sm" data-url=' . $url . ' type="number" id="order_' . $primary_key . '" value=' . $value . ' min="1"  onblur="updateOrder($(this).val(),' . $primary_key . ',' . $value . ')" />';
}

function feature_starsEntertainment($value, $fieldname, $primary_key, $row, $xcrud)
{
    $url = base_url() . "entertainment/entertainmentajaxcalls/update_featured";
    if ($value == "yes") {
        return "<span class='star fa fa-star' onclick='fstar(\"$url\",\"no\",\"$primary_key\")' id=\"$primary_key\"  style='cursor: pointer;'></span>";
    } else {
        return "<span class='fa fa-star-o fstar' onclick='fstar(\"$url\",\"yes\",\"$primary_key\")' id=\"$primary_key\" style='cursor: pointer;' ></span>";
...
//Mod #3  Restaurant
function restaurantGallery($value, $fieldname, $primary_key, $row, $xcrud)
{
    $photocounts = pt_RestaurantPhotosCount($primary_key);
    $CI = get_instance();
    $role = $CI->session->userdata('pt_role');
    if ($role != "supplier") {
        $role = "admin";
    }
    return "<a href=" . base_url() . $role . "/restaurant/gallery/" . $value . ">Upload (" . $photocounts . ")</a>";
}

function orderInputRestaurant($value, $fieldname, $primary_key, $row, $xcrud)
{
    $url = base_url() . "restaurant/restaurantajaxcalls/update_restaurant_order";

    return '<input class="form-control input-sm" data-url=' . $url . ' type="number" id="order_' . $primary_key . '" value=' . $value . ' min="1"  onblur="updateOrder($(this).val(),' . $primary_key . ',' . $value . ')" />';
}

function feature_starsRestaurant($value, $fieldname, $primary_key, $row, $xcrud)
{
    $url = base_url() . "restaurant/restaurantajaxcalls/update_featured";
    if ($value == "yes") {
        return "<span class='star fa fa-star' onclick='fstar(\"$url\",\"no\",\"$primary_key\")' id=\"$primary_key\"  style='cursor: pointer;'></span>";
    } else {
        return "<span class='fa fa-star-o fstar' onclick='fstar(\"$url\",\"yes\",\"$primary_key\")' id=\"$primary_key\" style='cursor: pointer;' ></span>";
...
//Mod #4
function spaGallery($value, $fieldname, $primary_key, $row, $xcrud)
{
    $photocounts = pt_SpaPhotosCount($primary_key);
    $CI = get_instance();
    $role = $CI->session->userdata('pt_role');
    if ($role != "supplier") {
        $role = "admin";
    }
    return "<a href=" . base_url() . $role . "/spa/gallery/" . $value . ">Upload (" . $photocounts . ")</a>";
}

function orderInputSpa($value, $fieldname, $primary_key, $row, $xcrud)
{
    $url = base_url() . "spa/spaajaxcalls/update_spa_order";

    return '<input class="form-control input-sm" data-url=' . $url . ' type="number" id="order_' . $primary_key . '" value=' . $value . ' min="1"  onblur="updateOrder($(this).val(),' . $primary_key . ',' . $value . ')" />';
}

function feature_starsSpa($value, $fieldname, $primary_key, $row, $xcrud)
{
    $url = base_url() . "spa/spaajaxcalls/update_featured";
    if ($value == "yes") {
        return "<span class='star fa fa-star' onclick='fstar(\"$url\",\"no\",\"$primary_key\")' id=\"$primary_key\"  style='cursor: pointer;'></span>";
    } else {
        return "<span class='fa fa-star-o fstar' onclick='fstar(\"$url\",\"yes\",\"$primary_key\")' id=\"$primary_key\" style='cursor: pointer;' ></span>";
...
//Mod #5
function weddingGallery($value, $fieldname, $primary_key, $row, $xcrud)
{
    $photocounts = pt_WeddingPhotosCount($primary_key);
    $CI = get_instance();
    $role = $CI->session->userdata('pt_role');
    if ($role != "supplier") {
        $role = "admin";
    }
    return "<a href=" . base_url() . $role . "/wedding/gallery/" . $value . ">Upload (" . $photocounts . ")</a>";
}

function orderInputWedding($value, $fieldname, $primary_key, $row, $xcrud)
{
    $url = base_url() . "wedding/weddingajaxcalls/update_wedding_order";

    return '<input class="form-control input-sm" data-url=' . $url . ' type="number" id="order_' . $primary_key . '" value=' . $value . ' min="1"  onblur="updateOrder($(this).val(),' . $primary_key . ',' . $value . ')" />';
}

function feature_starsWedding($value, $fieldname, $primary_key, $row, $xcrud)
{
    $url = base_url() . "wedding/weddingajaxcalls/update_featured";
    if ($value == "yes") {
        return "<span class='star fa fa-star' onclick='fstar(\"$url\",\"no\",\"$primary_key\")' id=\"$primary_key\"  style='cursor: pointer;'></span>";
    } else {
        return "<span class='fa fa-star-o fstar' onclick='fstar(\"$url\",\"yes\",\"$primary_key\")' id=\"$primary_key\" style='cursor: pointer;' ></span>";
...
function checkTableType($value, $fieldname, $primary_key, $row, $xcrud)
{
    if ($value == 'Y') {
        return 'pt_guest_accounts';
    } else {
        return 'pt_accounts';
    }
}

function checkIdType($value, $fieldname, $primary_key, $row, $xcrud)
{
    if ($value == 'Y') {
        return 'guest_id';
    } else {
...
function checkGuest($value, $fieldname, $primary_key, $row, $xcrud)
{
    if ($value == 'Y') {
        return 'Y';
    } else {
...
function bookingName($value, $fieldname, $primary_key, $row, $xcrud)
{
    //return 'Data is ' . $value;
    $ci = get_instance();
    $ci->db->select('booking_id,booking_member_id,booking_guest_id,is_guest');
    $ci->db->where('booking_id', $value);
    $res = $ci->db->get('pt_bookings')->result();

    if ($res[0]->is_guest == 'Y') {
        $table = 'pt_guest_accounts';
        $id = 'guest_id';
        $field = $res[0]->booking_guest_id;
    } else {
        $table = 'pt_accounts';
        $id = 'accounts_id';
        $field = $res[0]->booking_member_id;
    }

    $ci->db->select('ai_first_name,ai_last_name');
    $ci->db->where($id, $field);
    $q = $ci->db->get($table)->result();
    return '( ' . $value .' ) '. $q[0]->ai_first_name . ' ' . $q[0]->ai_last_name;
