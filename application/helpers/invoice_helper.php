...
if (!function_exists('invoiceDetails')) {
    function invoiceDetails($id, $ref, $isGuest, $reviewData = null)
    {
        $CI = get_instance();
        if ($isGuest == 'Y') {
	//		echo  "Guest Booking ";
            $CI->db->select('pt_bookings.*,pt_guest_accounts.ai_mobile,pt_guest_accounts.guest_id,pt_guest_accounts.ai_country,pt_guest_accounts.guest_email,pt_guest_accounts.ai_first_name,pt_guest_accounts.ai_last_name,pt_guest_accounts.ai_address_1,pt_guest_accounts.ai_address_2');
            $CI->db->where('pt_bookings.booking_id', $id);
            $CI->db->where('pt_bookings.booking_ref_no', $ref);
            $CI->db->join('pt_guest_accounts', 'pt_bookings.booking_guest_id = pt_guest_accounts.guest_id', 'left');
        } else {
		//	echo  "Member Booking ";
            ...
            $CI->db->where('pt_bookings.booking_id', $id);
            $CI->db->where('pt_bookings.booking_ref_no', $ref);
            $CI->db->join('pt_accounts', 'pt_bookings.booking_member_id = pt_accounts.accounts_id', 'left');
        }
        ...
        if ($isGuest == 'Y') {
            $accountEmail = $invoiceData[0]->guest_email;
        } else {
        ...
        $returnData = (object)array("id" => $invoiceData[0]->booking_id,
			...
			"paidAmount" => $invoiceData[0]->booking_total,
            ...
            "bookingUser" => $invoiceData[0]->booking_member_id,
            ...
            "member" => $invoiceData[0]->is_guest,
            "hotelname" => $invoiceData[0]->booking_customer_hotel_name,
            "hotel_checkin" => fromDbToAppFormatDate($invoiceData[0]->booking_customer_checkin),
            "hotel_checkout" => fromDbToAppFormatDate($invoiceData[0]->booking_customer_checkout)
...
if (!function_exists('pt_get_einvoice_details')) {
    function pt_get_einvoice_details($id, $itid, $member)
    {
        $CI = get_instance();
        if ($member == 'Y') {
            $CI->db->select('pt_ean_booking.*,pt_guest_accounts.ai_mobile,pt_guest_accounts.guest_id,pt_guest_accounts.ai_country,pt_guest_accounts.guest_email,pt_guest_accounts.ai_first_name,pt_guest_accounts.ai_last_name,pt_guest_accounts.ai_address_1,pt_guest_accounts.ai_address_2');
            $CI->db->where('pt_ean_booking.book_id', $id);
            $CI->db->where('pt_ean_booking.book_itineraryid', $itid);
            $CI->db->join('pt_guest_accounts', 'pt_ean_booking.book_user = pt_guest_accounts.guest_id', 'left');
        } else {
...
if (!function_exists('paymentLog')) {
    function paymentLog($result,$object, $location, $code, $message, $ref , $fullname ,$bookid,$datetime,$token,$process,$os,$browser,$ip,$payment_gateway)
    {
        $CI = get_instance();
        $data = array(
            'result' => $result,
            ...
            'token_created' => $token,
            'omise_process' => $process,
            'os' => $os,
            'browser' => $browser,
            'ip' => $ip,
            'payment_gateway' => $payment_gateway
        );
        $CI->db->insert('pt_payment_log', $data);

    }
}

if (!function_exists('paymentProcess')) {
    function paymentProcess($result,$object, $transaction, $payment_gateway, $fullname, $amount, $currency , $invoiceid,$invoiceref,$card,$expiration_month,$expiration_year,$brand ,$os ,$browser ,$ip ,$datetime ,$ref_transaction)
    {
        $CI = get_instance();
        $data = array(
            'object' => $result,
