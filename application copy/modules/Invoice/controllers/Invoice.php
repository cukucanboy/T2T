...
class Invoice extends MX_Controller
{
    ...
    public function index()
    {
        ...
        if (!empty ($ebookingid)) {
            $this->data['invoice'] = pt_get_einvoice_details($ebookingid, $bookingref, $member);
...
//   $this->theme->partial('invoice',$this->data);
            ...
            $this->data['invoice'] = invoiceDetails($bookingid, $bookingref, $member);
    ...
    function getGatewaylink($bookingid, $bookingref, $isGuest)
    {
        $this->load->helper('invoice');

        if ($this->input->is_ajax_request()) {
            if (!empty($_POST)) {
                $invoicdata = invoiceDetails($bookingid, $bookingref ,$isGuest);
                $this->load->model('Admin/Payments_model');
