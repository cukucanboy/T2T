...
//set tour id by id
    ...
    function tour_details($tourid = null, $date = null)
    {
        ...
        if (!empty($details[0]->tour_nearby_related)) {
            $nearbyrtour = $details[0]->tour_nearby_related;
        } else {
            $nearbyrtour = "";
        }
        ...
        $nearbyrelatedTour = $this->getNearbyRelated($nearbyrtour);

var_dump($nearbyrelatedTour);
        ...
        $ogimg = $this->ogimg($details[0]->tour_id);
        ...
        $detailResults = (object)array(
          ...
          'nearbyrelatedItems' => $nearbyrelatedTour,
...
// get tour images
    ...
    function ogimg($tourid)
    {
        ...
        if (empty($res)) {
            ...
            $result = 'uploads/images/tours/slider/thumbs/' . $res[0]->timg_image;
        }
        return $result;
    }

    function get_metaImg($defimg, $tourid = null)
    {
        if (empty($tourid)) {
            $tourid = $this->tourid;
        }
...
//getNearby
    ...
    function getNearbyRelatedTour($tour)
    {
      echo "<pre>";
      print_r($tour);
      print_r($nearby);
      echo "</pre>";
        ...
        if (!empty($tour)) {
            foreach ($tour as $t) {
                $resulttour[] = (object)array('tour_id' => $t);
            }
        }
        $result = $this->getLimitedResultObject($resulttour);
        return $result;
    }
    ...
    function getNearbyRelated($key)
    {
        $items = json_decode($key);
        $result = array();
        foreach ($items as $keys => $values)
        {
          $result[] = $this->getLimitedResultObject2($values->id ,$values->module);
        }
    ...
    function getLimitedResultObject2($id ,$module)
    {
  ...
  echo "</pre>";
      ...
      $curr = $this->ci->currconverter;
        ...
        //$this->Hotels->Hotels_lib->hotel_short_details();
        $this->db->select('*');
    ...
    echo "</pre>";
          ...
          $price = $curr->convertPrice($adultprice);
          if (!empty($this->title)) {
              $result = (object)array('id' => $this->hotelid, 'title' => $this->title, 'slug' => base_url() . 'hotels/' . $this->slug, 'thumbnail' => $this->thumbnail, 'stars' => pt_create_stars($this->stars), 'starsCount' => $this->stars, 'location' => $this->location, 'price' => $price, 'currCode' => $curr->code, 'currSymbol' => $curr->symbol, 'latitude' => $this->latitude, 'longitude' => $this->longitude, 'avgReviews' => $avgReviews,);
          }
        } 
        elseif ($module == 'car') {
          $this->set_id($id);
          //$this->Cars_lib->car_short_details($id);
          $adultprice = $this->adultPrice * $this->adults;
          $childprice = $this->childPrice;
          $infantprice = $this->infantPrice;
          ...
          $price = $curr->convertPrice($adultprice);
          if (!empty($this->title)) {
              $result = (object)array('id' => $this->carid, 'title' => $this->title, 'slug' => base_url() . 'cars/' . $this->slug, 'thumbnail' => $this->thumbnail, 'stars' => pt_create_stars($this->stars), 'starsCount' => $this->stars, 'location' => $this->location, 'price' => $price, 'currCode' => $curr->code, 'currSymbol' => $curr->symbol, 'latitude' => $this->latitude, 'longitude' => $this->longitude, 'avgReviews' => $avgReviews,);
          }
        }
...
*/
      ...
      if (!empty($tours)) {
            foreach ($tours as $t) {
                ...
                $price = $curr->convertPrice($adultprice);
                if (!empty($this->title)) {
                    $result[] = (object)array('id' => $this->tourid,'id' => $this->tourid, 'title' => $this->title, 'slug' => base_url() . 'tours/' . $this->slug, 'thumbnail' => $this->thumbnail, 'stars' => pt_create_stars($this->stars), 'starsCount' => $this->stars, 'location' => $this->location, 'price' => $price, 'currCode' => $curr->code, 'currSymbol' => $curr->symbol, 'latitude' => $this->latitude, 'longitude' => $this->longitude, 'avgReviews' => $avgReviews,);
                }
            }
        }
        return $result;
...
elseif ($module == 'wedding') {
    ...
    echo "</pre>";
        ...
        if (!empty($tours)) {
            foreach ($tours as $t) {
                ...
                $price = $curr->convertPrice($adultprice);
                if (!empty($this->title)) {
                    $result[] = (object)array('moduleid' => $module,'id' => $this->tourid, 'title' => $this->title, 'slug' => base_url() . 'tours/' . $this->slug, 'thumbnail' => $this->thumbnail, 'stars' => pt_create_stars($this->stars), 'starsCount' => $this->stars, 'location' => $this->location, 'price' => $price, 'currCode' => $curr->code, 'currSymbol' => $curr->symbol, 'latitude' => $this->latitude, 'longitude' => $this->longitude, 'avgReviews' => $avgReviews,);
                }
            }
        }
        return $result;
    ...
    // add by poykub
    function conv_split($data)
    {
      $items = json_decode($data);
      foreach ($items as $key => $value)
      {
        //echo $value->id .'+'. $value->module .'<br />';
        //$md = $this->Tours_model->chkmodulerr($value->module);
        //$list .= $value->id.'' .$md .',';
        $list .= $value->id .',';
      }
      //echo "<pre>";
