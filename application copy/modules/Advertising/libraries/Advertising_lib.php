...
//set advertising id by id
    ...
    function advertising_details($advertisingid = null, $date = null)
    {
        ...
        $detailResults = (object)array(
          ...
          'relatedItems' => $relatedAdvertising,
          'nearbyrelatedItems' => $nearbyrelatedAdvertising,
          ...
          'advertisingNights' => $advertisingNights,
          'totalDeposit' => $depositAmount,
          'mapAddress' => $details[0]->advertising_mapaddress);
        return $detailResults;
    }

    function advertising_short_details($advertisingid = 0)
    {
        if (empty($advertisingid)) {
            $advertisingid = $this->advertisingid;
        }
...
//get country and city name for url slug
        ...
        $countryName = url_title($locationInfoUrl->country, 'dash', true);
        $cityName = url_title($locationInfoUrl->city, 'dash', true);
        $this->slug = $countryName . '/' . $cityName . '/' . $details[0]->advertising_slug . $this->urlVars;
...
// Get Advertising updated Price on changing adults, child and infant count.
    ...
    function get_title($deftitle, $advertisingid = 0)
    {
        if (empty($advertisingid)) {
            $advertisingid = $this->advertisingid;
        }
    ...
    function get_description($defdesc, $advertisingid = 0)
    {
        if (empty($advertisingid)) {
            $advertisingid = $this->advertisingid;
        }
    ...
    function get_policy($defpolicy, $advertisingid = 0)
    {
        if (empty($advertisingid)) {
            $advertisingid = $this->advertisingid;
        }
...
// advertising  Reviews Averages
    function advertisingReviewsAvg($advertisingid = 0)
    {
        if (empty($advertisingid)) {
            $advertisingid = $this->advertisingid;
        }
...
// Advertising visiting Cities
    ...
    function is_featured($advertisingid = 0)
    {
        if (empty($advertisingid)) {
            $advertisingid = $this->advertisingid;
        } else {
