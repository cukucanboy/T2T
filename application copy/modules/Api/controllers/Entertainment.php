...
class Entertainment extends REST_Controller {
    ...
    function __construct() {
        ...
        $lang = $this->get('lang');
				$this->Entertainment_lib->set_lang($lang);
    }

    function list_get() {
        $offset = $this->get('offset');
        $list = $this->Entertainment_lib->show_entertainment($offset);
        $totalPages = ceil($list['paginationinfo']['totalrows'] / $list['paginationinfo']['perpage']);
        if (!empty ($list['all_entertainment'])) {
         $this->response(array('response' => $list['all_entertainment'], 'error' => array('status' => FALSE,'msg' => ''), 'totalPages' => $totalPages), 200);
        }
        else {
           $this->response(array('response' => '', 'error' => array('status' => TRUE,'msg' => 'Entertainment could not be found')), 200);
        }
    }

    function locations_get(){
        $locations = $this->Entertainment_lib->getLocationsList();
        $locArray = array();
    ...
    function entertainmenttypes_get(){
        $entertainmenttypes = $this->Entertainment_lib->entertainmentTypes();


         if (!empty ($entertainmenttypes)) {
              $this->response(array('response' => $entertainmenttypes, 'error' => array('status' => FALSE,'msg' => '')), 200);
        }
        else {
     ...
     function suggestions_get(){
        ...
        $suggestions = $this->Entertainment_lib->suggestionResults($query);
    ...
    function details_get() {
        ...
        $details['entertainment'] = $this->Entertainment_lib->entertainment_details($id, $date);
        $details['entertainment']->desc = strip_tags($details['entertainment']->desc);

        if (pt_is_module_enabled('reviews')) {
                        $details['reviews'] = $this->Entertainment_lib->entertainment_reviews_for_api($details['entertainment']->id);
                        $details['avgReviews'] = $this->Entertainment_lib->entertainmentReviewsAvg($details['entertainment']->id);
        }
    ...
    function search_get() {
                ...
                $details = $this->Entertainment_lib->search_entertainment($cityid , $offset);
                $totalPages = ceil($details['paginationinfo']['totalrows'] / $details['paginationinfo']['perpage']);

                if (!empty ($details['all_entertainment'])) {
                  $this->response(array('response' => $details['all_entertainment'], 'error' => array('status' => FALSE,'msg' => ''), 'totalPages' => $totalPages), 200);
                }
                else {
    ...
    function show_get($param, $vars = null) {
        ...
        foreach($arr as $key => $val){
         ...
         $url = base_url()."api/entertainment/".$param."?".$arrstr;
