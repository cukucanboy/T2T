...
// get hotel images
    ...
    function ogimg($hotelid)
    {
        if (empty($hotelid)) {
            $hotelid = $this->hotelid;
        }
        $this->db->where('himg_hotel_id', $hotelid);
        $this->db->where('himg_order', '1');
        $res = $this->db->get('pt_hotel_images')->result();
        if (empty($res)) {
            $result = 'uploads/global/favicon.png';
        } else {
            $result =  'uploads/images/hotels/slider/thumbs/'.$res[0]->himg_image;
        }
        return $result;
    }
...
// Room Price
    ...
    function hotel_details()
    {
        ...
        if (!empty($details[0]->hotel_nearby_related)) {
            $nearbyhotels = explode(",", $details[0]->hotel_nearby_related);
        } else {
            $nearbyhotels = "";
        }

        $relatedHotels = $this->getRelatedHotels($rhotels);
        $nearbyrelatedHotel = $this->getNearbyRelatedHotels($nearbyhotels);
...
//	$this->bestprice = $details[0]->hotel_best_price;
    ...
    function get_metaImg($defimg, $hotelid = null)
    {
        if (empty($hotelid)) {
            $hotelid = $this->hotelid;
        }
...
// $unavail =  pt_is_room_unvailable($shortdetails[0]->room_id,$this->checkin,$this->checkout);
    ...
    function getNearbyRelatedHotels($hotels)
    {
        $resulthotel = array();
        $result = array();
        if (!empty($hotels)) {
            foreach ($hotels as $t) {
                $resulthotel[] = (object)array('hotel_id' => $t);
            }
        }
        $result = $this->getLimitedResultObject($resulthotel);
        return $result;
    }
