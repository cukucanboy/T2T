...
//process booking
    ...
    function processBookinglogged()
    {
        ...
        echo json_encode($this->Bookings_model->do_booking($user, NULL, 'N'));
    }
...
//Destination
    public function createDestinationsPermalink()
    {
        $title = $this->input->post('title');
        $postid = $this->input->post('postid');
        ...
        if (empty ($lastid)) {
            ...
            $postlastid = $lastid[0]->destination_id + 1;
        }
        $slug = create_url_slug($title);
        $this->db->select("destination_id");
        ...
        if ($postid > 0) {
            $this->db->where('destination_id !=', $destinationid);
        }
    ...
    public function createWishPermalink()
    {
        $title = $this->input->post('title');
