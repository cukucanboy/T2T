...
//hotels assignment
        ...
        $advertisingmod = $this->ptmodules->is_mod_available_enabled("advertising");
        if ($advertisingmod) {
            $this->load->model('Advertising/Advertising_model');
            $this->Advertising_model->assignAdvertising($this->input->post('advertising'), $id);
        }
...
//cars assignment
    ...
    function update_profile_guest($id, $filename = null)
    {

        $now = date("Y-m-d H:i:s");
        $data = array('ai_title' => "", 'ai_first_name' => $this->input->post('firstname'), 'ai_last_name' => $this->input->post('lastname'), 'ai_city' => $this->input->post('city'), 'ai_state' => $this->input->post('state'), 'ai_country' => $this->input->post('country'), 'ai_address_1' => $this->input->post('address1'), 'ai_address_2' => $this->input->post('address2'), 'ai_mobile' => $this->input->post('phone'), 'ai_fax' => $this->input->post('fax'), 'ai_postal_code' => $this->input->post('zip'), 'guest_updated_at' => $now);
        $this->db->where('guest_id', $id);
        $this->db->update('pt_guest_accounts', $data);
        $password = $this->input->post('password');
        if (!empty ($password)) {
            $this->change_password($id);
        }
...
// get profile details
    ...
    function get_guest_profile_details($id)
    {
        $this->db->where('guest_id', $id);
        $q = $this->db->get('pt_guest_accounts')->result();
        return $q;
    }
...
// Signup account
    ...
    // Guest booking
    function guest_booking($type, $status)
    {
        $address = $this->input->post('address');
        if (empty($address)) {
            $address = "";
        ...
        $this->db->insert('pt_guest_accounts', $data);
        $id = $this->db->insert_id();
        //if ($type == "guest") {
        //    if($status == "yes"){
        $this->session->set_userdata('pt_logged_guest', $id);
        $this->session->set_userdata('fname', $this->input->post('firstname'));
        $this->session->set_userdata('pt_role', $type);
        //   }
...
// List all bookings on account my bookings page
   ...
   pt_bookings.booking_expiry,pt_reviews.review_overall,pt_reviews.review_date,pt_reviews.review_comment,pt_reviews.review_itemid');
        $this->db->where('pt_bookings.booking_member_id', $id);
    ...
    // List all points on account my bookings page
    function get_my_points($id = 33)
    {
        $data = array();
        $this->db->where(array('accounts_id' => $id, 'is_approve' => 'Y'));
        $this->db->order_by('service_date', 'desc');
        $query = $this->db->get('pt_accounts_reward');
        $result = $query->result();
        if (!empty ($result)) {
            return $result;
        }
