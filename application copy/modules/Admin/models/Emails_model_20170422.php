...
//send email to customer
    function sendEmail_customer($details, $sitetitle ,$isGuest)
    {
        ...
        $invoicelink = "<a href=" . base_url() . "invoice?id=" . $invoiceid . "&sessid=" . $refno . "&member=" . $isGuest ." >" . base_url() . "invoice?id=" . $invoiceid . "&sessid=" . $refno . "</a>";
...
//send email to Owner
    function sendEmail_owner($details, $sitetitle, $isGuest)
    {
        ...
        $message .= "<br> To view Invoice visit at: <a href=" . base_url() . "invoice?id=" . $invoiceid . "&sessid=" . $refno . "&member=" . $isGuest ." >" . base_url() . "invoice?id=" . $invoiceid . "&sessid=" . $refno . "</a>";
        $message .= $this->mailFooter;
...
// get owner email
    function ownerEmail($type, $id)
    {
        ...
        elseif ($type == "advertising") {
            $this->db->select('advertising_email');
            $this->db->where('advertising_id', $id);
            $q = $this->db->get('pt_advertising')->result();
            $email = $q[0]->tour_advertising;
        } elseif ($type == "cars") {
...
// get supplier Email
    function supplierEmail($type, $id)
    {
        ...
        elseif ($type == "advertising") {
            $this->db->select('advertising_owned_by');
            $this->db->where('advertising_id', $id);
            $q = $this->db->get('pt_advertising')->result();
            $email = $this->get_user_email($q[0]->advertising_owned_by);
        } //End Modules
