...
if (!function_exists('pt_default_restaurant_image')) {
		function pt_default_restaurant_image($restaurantid) {
				$CI = get_instance();
				$CI->load->model('Restaurant/Restaurant_model');
				$res = $CI->Advertising_model->default_restaurant_img($restaurantid);
				return $res;
		}
}
...
if (!function_exists('pt_default_activity_image')) {
		function pt_default_activity_image($activityid) {
				$CI = get_instance();
				$CI->load->model('Activity/Activity_model');
				$res = $CI->Activity_model->default_activity_img($activityid);
				return $res;
		}
}

if (!function_exists('pt_default_spa_image')) {
		function pt_default_spa_image($spaid) {
				$CI = get_instance();
				$CI->load->model('Spa/Spa_model');
				$res = $CI->Spa_model->default_spa_img($spaid);
				return $res;
		}
}
		...
		function pt_pendings_count($type, $module = null) {
				...
				if ($type = 'images') {
						...
						elseif ($module == 'advertising') {
								$counts['images']['advertising'] = $CI->Misc_model->count_pending_advertising_images();
						}
						//Activity
						elseif ($module == 'activity') {
								$counts['images']['activity'] = $CI->Misc_model->count_pending_activity_images();
						}

						// Modules Restaurant
						elseif ($module == 'restaurant') {
								$counts['images']['restaurant'] = $CI->Misc_model->count_pending_restaurant_images();
						}

						// Wedding
						elseif ($module == 'wedding') {
								$counts['images']['wedding'] = $CI->Misc_model->count_pending_wedding_images();
						}

						// Entertainment
						elseif ($module == 'entertainment') {
								$counts['images']['entertainment'] = $CI->Misc_model->count_pending_entertainment_images();
						}

						// Spa
						elseif ($module == 'spa') {
								$counts['images']['spa'] = $CI->Misc_model->count_pending_spa_images();
						}
...
elseif ($module == "restaurant") {
		if ($type == 'default') {
				return PT_RESTAURANT_SLIDER;
		}
		elseif ($type == 'slider') {
				return PT_RESTAURANT_SLIDER;
		}
}
//end restaurant Modules

//Mod #4
elseif ($module == "entertainment") {
		if ($type == 'default') {
				return PT_ENTERTAINMENT_SLIDER;
		}
...
//add Modules  Advertising
$advertisingimgscount = pt_pendings_count('images', 'advertising');
$ticounts = $advertisingimgscount['images']['advertising'];
$total['img_advertising'] = $ticounts;
$reviewsadvertisingrcount = pt_pendings_count('reviews', 'advertising');
$trevcounts = $reviewsadvertisingcount['reviews']['advertising'];
$total['rev_advertising'] = $trevcounts;


//Mod #1
$activityimgscount = pt_pendings_count('images', 'activity');
$ticounts = $activityimgscount['images']['activity'];
$total['img_activity'] = $ticounts;
$reviewsactivityrcount = pt_pendings_count('reviews', 'activity');
$trevcounts = $reviewsactivitycount['reviews']['activity'];
$total['rev_activity'] = $trevcounts;

//Mod #2
$entertainmentimgscount = pt_pendings_count('images', 'entertainment');
$ticounts = $entertainmentimgscount['images']['entertainment'];
$total['img_entertainment'] = $ticounts;
$reviewsentertainmentrcount = pt_pendings_count('reviews', 'entertainment');
$trevcounts = $reviewsentertainmentcount['reviews']['entertainment'];
$total['rev_entertainment'] = $trevcounts;

//Mod #3
$restaurantimgscount = pt_pendings_count('images', 'restaurant');
$ticounts = $restaurantimgscount['images']['restaurant'];
$total['img_restaurant'] = $ticounts;
$reviewsrestaurantrcount = pt_pendings_count('reviews', 'restaurant');
$trevcounts = $reviewsrestaurantcount['reviews']['restaurant'];
$total['rev_restaurant'] = $trevcounts;

//Mod #4
$spaimgscount = pt_pendings_count('images', 'spa');
$ticounts = $spaimgscount['images']['spa'];
$total['img_spa'] = $ticounts;
$reviewssparcount = pt_pendings_count('reviews', 'spa');
$trevcounts = $reviewsspacount['reviews']['spa'];
$total['rev_spa'] = $trevcounts;

//Mod #5
$weddingimgscount = pt_pendings_count('images', 'wedding');
$ticounts = $weddingimgscount['images']['wedding'];
$total['img_wedding'] = $ticounts;
$reviewsweddingrcount = pt_pendings_count('reviews', 'wedding');
$trevcounts = $reviewsweddingcount['reviews']['wedding'];
$total['rev_wedding'] = $trevcounts;
...
//add Modules Advertising
if (!function_exists('pt_AdvertisingPhotosCount')) {
		function pt_AdvertisingPhotosCount($advertisingid) {
				$CI = get_instance();
				$CI->load->model('Advertising/Advertising_model');
				$res = $CI->Advertising_model->photos_count($advertisingid);
				return $res;
		}
}
//Mod #1
if (!function_exists('pt_ActivityPhotosCount')) {
		function pt_ActivityPhotosCount($activityid) {
				$CI = get_instance();
				$CI->load->model('Activity/Activity_model');
				$res = $CI->Activity_model->photos_count($activityid);
				return $res;
		}
}

#Mod #2
if (!function_exists('pt_EntertainmentPhotosCount')) {
		function pt_EntertainmentPhotosCount($entertainmentid) {
				$CI = get_instance();
				$CI->load->model('Entertainment/Entertainment_model');
				$res = $CI->Entertainment_model->photos_count($entertainmentid);
				return $res;
		}
}

//Mod #3 Restaurant
if (!function_exists('pt_RestaurantPhotosCount')) {
		function pt_RestaurantPhotosCount($restaurantid) {
				$CI = get_instance();
				$CI->load->model('Restaurant/Restaurant_model');
				$res = $CI->Restaurant_model->photos_count($restaurantid);
				return $res;
		}
}

//Mod #4  Spa
if (!function_exists('pt_SpaPhotosCount')) {
		function pt_SpaPhotosCount($spaid) {
				$CI = get_instance();
				$CI->load->model('Spa/Spa_model');
				$res = $CI->Spa_model->photos_count($spaid);
				return $res;
		}
}
...
//Mod #5  Wedding
if (!function_exists('pt_WeddingPhotosCount')) {
		function pt_WeddingPhotosCount($weddingid) {
				$CI = get_instance();
				$CI->load->model('Wedding/Wedding_model');
				$res = $CI->Wedding_model->photos_count($weddingid);
				return $res;
		}
}
