...
$reviewstourcount = pt_pendings_count('reviews', 'advertising');
$trevcounts = $reviewstourcount['reviews']['advertising'];
...
if (!function_exists('pt_carPhotosCount')) {
    ...
    function checkUrlParams($method, $params = array(),$isValidLang) {
    	...
    	if($paramCount > 0){
			$result->destination = $result->param1;
			$result->wishSlug = $result->param2;
    	$result->countrySlug = $result->param3;
    	$result->location = $result->param4;
    	$result->itemSlug = $result->param5;
    	$urlBase .= '/'.$result->destination;


    	if(is_numeric($result->destination)){
    	$result->offset = $result->destination;
    	$result->uriSegment = 4;
    	}else{
    	$urlBase .= '/'.$result->destination;
		...
		$result->destinationSlug = $method;
		if($paramCount > 0){
		$result->wishSlug = $result->param1;
